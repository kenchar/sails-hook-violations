/**
 * @Filename update.js
 * @Description 扩展原生的updateOne,支持model内自定义校验信息
...
const DEFERRED_METHODS = getQueryModifierMethods('updateOne');
...
module.exports = model => {
    ...
    let sailsUpdateOne = model.updateOne;

    let sailsUpdateOneCall = (criteria, valuesToSet, explicitCbMaybe, metaContainer) => {
    ...
    let updateOne = (criteria, valuesToSet, explicitCbMaybe, metaContainer) => {

        let WLModel = model;
        let modelIdentity = model.identity;

        return new parley(function (done) {
            sailsUpdateOneCall(this._wlQueryInfo.criteria, this._wlQueryInfo.valuesToSet, done, this._wlQueryInfo.meta);
